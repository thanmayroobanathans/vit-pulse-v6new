...
function classify(){
  ...
  $("ranking").innerHTML=scored.slice(0,7).map((x,n)=>
    `<div class="rank"><span>${String(n+1).padStart(2,"0")}</span><b>${x.a[0]}</b><span>${(x.p*100).toFixed(1)}%</span></div>`
  ).join("");

  const campus=[
    ["SJT / AMAZON",Math.round(state.profile[2]*65+state.profile[0]*35)+"%"],
    ["SOCIAL ENERGY",Math.round(state.profile[2]*100)+"%"],
    ["NOVELTY SEEKING",Math.round(state.profile[4]*70+state.profile[0]*30)+"%"],
    ["STRUCTURE",Math.round(state.profile[1]*100)+"%"],
    ["INTELLECTUAL CURIOSITY",Math.round(state.profile[7]*100)+"%"]
  ];
  $("campusStats").innerHTML=campus.map(x=>
    `<div class="campus"><span>${x[0]}</span><b>${x[1]}</b></div>`
  ).join("");

  // Put a useful uncertainty statement into the existing machine-status area.
  if($("machineNote")){
...
function renderBars(id,data){$(id).innerHTML=data.map(x=>`<div class="barrow"><span>${x[0]}</span><div class="bar"><i style="width:${Math.round(x[1]*100)}%"></i></div><span>${Math.round(x[1]*100)}</span></div>`).join("")}
function renderFact(){const f=FACTS[state.factIndex%FACTS.length];$("factLabel").textContent=f[0];$("fact").textContent=f[1]}
$("begin").onclick=()=>{state.name=$("userName").value.trim()||"Anonymous Subject";$("resultName").textContent=state.name;state.i=0;state.profile=new Array(11).fill(.5);state.answers=[];show("quiz");renderQuestion()};
$("again").onclick=()=>$("begin").click();
$("nextFact").onclick=()=>{state.factIndex=(state.factIndex+1+Math.floor(Math.random()*3))%FACTS.length;renderFact()};
function resultPayload(){return {name:state.name,archetype:$("archetype").textContent,score:$("matchScore").textContent,code:$("archetypeCode").textContent,confidence:state.confidence?Math.round(state.confidence*100):null,version:"v4"}}
function makeShareUrl(){const p=resultPayload();const data=btoa(unescape(encodeURIComponent(JSON.stringify(p))));return location.href.split("#")[0]+"#result="+data}
$("share").onclick=async()=>{const t=`${state.name} was classified as ${$("archetype").textContent} — ${$("matchScore").textContent}% match. The machine has spoken.`;const u=makeShareUrl();try{if(navigator.share)await navigator.share({title:"VIT PULSE — Classification",text:t,url:u});else{await navigator.clipboard.writeText(t+" "+u);$("shareStatus").textContent="RESULT LINK COPIED."}}catch(e){$("shareStatus").textContent="SHARE CANCELLED."}};
$("shareData").onclick=async()=>{const u=makeShareUrl();$("shareBox").classList.add("active");$("shareLink").value=u;try{await navigator.clipboard.writeText(u);$("shareStatus").textContent="RESULT LINK COPIED — SEND IT TO A FRIEND."}catch(e){}};
$("copyLink").onclick=async()=>{await navigator.clipboard.writeText($("shareLink").value);$("shareStatus").textContent="LINK COPIED."};
$("saveImage").onclick=async()=>{const card=document.querySelector(".hero-result");try{const canvas=document.createElement("canvas"),ctx=canvas.getContext("2d");canvas.width=1200;canvas.height=760;ctx.fillStyle="#080908";ctx.fillRect(0,0,1200,760);ctx.fillStyle="#ebe5d5";ctx.font="bold 46px Arial";ctx.fillText("VIT PULSE",70,90);ctx.fillStyle="#c94236";ctx.font="bold 78px Arial";ctx.fillText($("archetype").textContent,70,230);ctx.fillStyle="#ebe5d5";ctx.font="28px monospace";ctx.fillText(state.name,70,300);ctx.font="bold 80px Arial";ctx.fillText($("matchScore").textContent+"%",70,430);ctx.font="22px monospace";ctx.fillText("PSYCHOLOGICAL CLASSIFICATION // V4",70,510);ctx.fillText("JUNG-INSPIRED · STATISTICAL · FICTIONAL",70,550);const a=document.createElement("a");a.download="vit-pulse-result.png";a.href=canvas.toDataURL("image/png");a.click();$("shareStatus").textContent="RESULT CARD CREATED."}catch(e){$("shareStatus").textContent="IMAGE EXPORT FAILED."}};
$("sendData").onclick=async()=>{
 if(!$("optin").checked){$("sendStatus").textContent="OPT-IN REQUIRED. NOTHING WAS SENT.";return}
 // Endpoint intentionally blank in the static build. Set AGGREGATION_ENDPOINT
 // below to your own privacy-reviewed endpoint when deploying a backend.
 const AGGREGATION_ENDPOINT="";
 const payload={version:"v3",archetype:state.best.a[0],score:Math.round(state.match*1000)/1000,answersHash:await hash(JSON.stringify(state.answers)),createdAt:new Date().toISOString()};
 if(!AGGREGATION_ENDPOINT){$("sendStatus").textContent="LOCAL DEMO: aggregation endpoint is not configured, so nothing was sent.";return}
 try{await fetch(AGGREGATION_ENDPOINT,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(payload),keepalive:true});$("sendStatus").textContent="ANONYMOUS AGGREGATE SUBMITTED."}catch(e){$("sendStatus").textContent="SUBMISSION FAILED. NOTHING ELSE WAS RETRIED."}
};
async function hash(s){const b=new TextEncoder().encode(s),h=await crypto.subtle.digest("SHA-256",b);return [...new Uint8Array(h)].map(x=>x.toString(16).padStart(2,"0")).join("")}

function loadShared(){
 const m=location.hash.match(/^#result=(.+)$/); if(!m)return;
 try{
   const p=JSON.parse(decodeURIComponent(escape(atob(m[1]))));
   state.name=p.name||"Shared Subject";
   $("resultName").textContent=state.name;$("archetype").textContent=p.archetype||"UNKNOWN";$("archetypeCode").textContent=p.code||"SHARED";$("matchScore").textContent=p.score||"—";
   $("description").textContent="This result was shared with you. Run the machine yourself to receive a complete personal dossier.";
   $("jungStats").innerHTML="";$("bigFive").innerHTML="";$("ranking").innerHTML=`<div class="rank"><span>01</span><b>${p.archetype}</b><span>${p.score}%</span></div>`;$("campusStats").innerHTML="";
   state.factIndex=Math.floor(Math.random()*FACTS.length);renderFact();show("result");
 }catch(e){}
}
loadShared();

document.addEventListener("DOMContentLoaded",()=>{const n=localStorage.getItem("vp_name");if(n&&$("userName"))$("userName").value=n;});
